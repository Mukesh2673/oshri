...
//Add Profile //
export const addProfile = async (token: any, req: any) => {
    const decoded: any = await ListingUtilities.getDecoded(token);
          let bodyData: any = req.body;

    let isExist: any = await profileModel.findOne({ city: bodyData.city, email:bodyData.email, isDeleted: false });
    if (!isExist){
        // bodyData.createdBy = decoded.id;
        // bodyData.updatedBy = decoded.id;
        let result = await profileModel.create(bodyData);
...
export const getProfileList = async (token: any, body: any) => {
    // const decoded: any = await ListingUtilities.getDecoded(token);
    ...
    let totalRecords = await profileModel.find({ $and: query });
    let profileRes = await profileModel.find({ $and: query }).sort({ name: 'asc' }).skip(parseInt(skip)).limit(parseInt(limit));
    if (profileRes.length > 0) {
        return { responseCode: 200, responseMessage: 'Success', data: profileRes, totalRecord: totalRecords.length };
    } else {
        return { responseCode: 200, responseMessage: 'Success', data: profileRes, totalRecord: totalRecords.length };
